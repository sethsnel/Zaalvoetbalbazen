    ...
    <nav className={`navbar sticky-top navbar-expand-sm ${!isAdmin && 'd-none'} d-md-block`}>
      <div className='container-fluid position-relative'>
            ...
            {isAdmin && (
              <li className='nav-item dropdown'>
                ...
                <ul className='dropdown-menu'>
                  <li>
                    <DropdownItem name='Seizoenen' href='/seizoenen' currentPath={pathname} onClick={() => setExpandedMenu(false)} />
                  </li>
                  <li>
                    <DropdownItem name='Aanwezigheid' href='/aanwezigheid' currentPath={pathname} onClick={() => setExpandedMenu(false)} />
                  </li>
                  <li>
                    <DropdownItem name='Sessies' href='/sessie-management' currentPath={pathname} onClick={() => setExpandedMenu(false)} />
                  </li>
                </ul>
              </li>
            )}
        ...
        <div className='position-absolute end-0 top-0 me-3'>
          <Link href='/profiel' legacyBehavior>
            <Image
              src={profileUrl}
              height={40}
              width={40}
              className={`${styles.picture} cursor-pointer`}
              objectFit='cover'
              alt='Profile picture'
            />
          </Link>
...
function MenuItem({ name, href, currentPath, onClick }: { name: string; href: string; currentPath: string; onClick: () => void }) {
  if (currentPath === href) {
    return (
      <Link href={href} className='nav-link active' aria-current='page' onClick={onClick}>
        {name}
      </Link>
    )
  } else {
...
function DropdownItem({ name, href, currentPath, onClick }: { name: string; href: string; currentPath: string; onClick: () => void }) {
  if (currentPath === href) {
    return (
      <Link href={href} className='dropdown-item active' aria-current='page' onClick={onClick}>
        {name}
      </Link>
    )
  } else {
